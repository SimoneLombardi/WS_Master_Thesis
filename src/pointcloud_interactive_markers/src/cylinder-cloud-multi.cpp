    ...
    for(int i=0; i<=num_theta; ++i){
        ...
        for(int j=0; j<=num_steps_height; ++j){
            if(i%2==0 && j%2==0){
              ...
              pcl::PointXYZ pointpl;
              pcl::PointXYZ pointmn;
              pointpl.x = (radius * std::cos(theta)) + xshift;
              pointpl.y = radius * std::sin(theta);
              pointpl.z = z;

              pointmn.x = (radius * std::cos(theta)) - xshift;
              pointmn.y = radius * std::sin(theta);
              pointmn.z = z;
              
              cloud->points.push_back(pointpl);
              cloud->points.push_back(pointmn);
            }else if(i%2==1 && j%2==1){
              double z = (height / num_steps_height) * j;
              pcl::PointXYZ pointpl;
              pcl::PointXYZ pointmn;
